--- @class LocaleSna
LocaleSna = {}
LocaleSna.__index = LocaleSna

local function translateKey(phrase, ...)
...
--- @return Locale
function LocaleSna.new(_, opts)
    local self = setmetatable({}, LocaleSna)

    self.fallback = opts.fallbackLang and LocaleSna:new({
        warnOnMissing = false,
...
--- @return nil
function LocaleSna:extend(phrases, prefix)
...
--- @return string
function LocaleSna:locale(newLocale)
    if (newLocale) then
...
function LocaleSna:t(key, ...)
    ...
    if type(self.phrases[key]) == 'string' then
        ...
        if self.fallback then
            return self.fallback:t(key, ...)
        end
        result = key
    end

    if type(phrase) == 'string' then
        result = translateKey(phrase, ...)
    end
...
--- @param prefix string
function LocaleSna:delete(phraseTarget, prefix)
...
function InsertTranslation(translations, language)
    local convar
    ...
    if language == 'en' then
        Lang = Lang or LocaleSna:new({
            phrases = translations,
            warnOnMissing = true
        })
    else
        if GetConvar(convar, 'en') == language then
            Lang = LocaleSna:new({
                phrases = translations,
                warnOnMissing = true,
                fallbackLang = Lang,
